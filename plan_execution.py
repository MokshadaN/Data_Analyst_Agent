...
def execute_plan_v1(plan, questions , data_files, max_retries=3):
    ...
    if ok:
        return output  # already JSON string

    # Retry loop with feedback
    for _ in range(max_retries):
        print("[EXECUTE][RETRY] Code ")
        repair_prompt = _build_repair_prompt(system_prompt, plan, questions,data_files, code, error)
        print("[EXECUTE][RETRY] Built Repair Prompt and Calling Gemini API")
        code = gemini_call_for_code(*repair_prompt, None)
...
def _run_and_validate_json(code: str, timeout_sec: int = 180):
    """
